...
def preprocess(source):
    preprocessed = pd.DataFrame(index = range(1, max(source["ID"])+1), columns=["ID", "MakeText", "TypeName",
                                                                           "TypeNameFull", "ModelText", "ModelTypeText"])
    for i in range(1, max(source["ID"])+1):
        car_id = source[source["ID"] == i]
...
def normalize(preprocessed):
    ...
    for i in range(1, preprocessed.shape[0] + 1):
        ...
        # this would have to be checked with the supplier / customer. As there are null values in
        # the source, these must also be considered.
        try:
            consumption = preprocessed.loc[i, "ConsumptionTotalText"]
            if consumption[-2:] == "km":
                normalized.loc[i, "fuel_consumption_unit"] = "l_km_consumption"
            elif consumption == "null":
                normalized.loc[i, "fuel_consumption_unit"] = "null"
            elif consumption[-2:] == "/l":
        ...
        # As far as I can tell, these are all the normalizations that are needed / possible
    return normalized

# Step 3: Integration of the normalized data into the target data schema

def integrate(normalized):
    ...
    for i in range(1, normalized.shape[0] +1):
        ...
        # In some cases, the model text is blank. However, in those cases the model name will still be included in the type
        if normalized.loc[i, "ModelText"] != None:
            output.loc[i, "model"] = normalized.loc[i, "ModelText"]
        else:
            output.loc[i, "model"] = "null"
...
with pd.ExcelWriter(output_file) as writer:
    ...
    normalized.to_excel(writer, sheet_name='normalized', index = False)
